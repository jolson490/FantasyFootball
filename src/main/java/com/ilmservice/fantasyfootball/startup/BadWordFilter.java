...
@Component
public class BadWordFilter implements ApplicationListener<ApplicationReadyEvent> {
  ...
  public static String getCensoredText(final String input) {
    logger.debug("in getCensoredText(): input={}", input);
    ...
    logger.debug("inputToReturn: {}", inputToReturn);

    return inputToReturn;
  } // end getCensoredText

  private void loadBadWords() {
    logger.debug("in loadBadWords()");
    int readCounter = 0;
    try {
      ...
      while ((currentLine = reader.readLine()) != null) {
        ...
        try {
          if (1 == readCounter) {
            logger.trace("Skipping the first line in the spreadsheet: {}", currentLine);
            continue;
          }
          ...
          if (word.startsWith("-----")) {
            logger.trace("Skipping line that starts with minus signs: {}", currentLine);
            continue;
          }
          ...
          logger.error("Error processing line in file", except);
        }
      } // end while
    } catch (IOException except) {
      logger.error("Error creating reader", except);
    }

    logger.debug(
      "Loaded {} lines from the spreadsheet (words to consider censoring out) - and allBadWords size is {}.",
      readCounter, allBadWords.size());

    // logger.trace("words: {}", words);

    //for (@SuppressWarnings("rawtypes") Map.Entry entry : allBadWords.entrySet()) {
